...
func initMemberLoan(router *mux.Router) {
	...
	router.HandleFunc("/api/viewAllMemberLoansGivenMemberId", viewAllMemberLoansGivenMemberId).Methods("POST", "OPTIONS")
}
...
func viewAllMemberLoansGivenMemberId(w http.ResponseWriter, r *http.Request) {
	query(w, r, `SELECT m.*, date_format(m.req_date,'%Y-%m-%d') AS req_date, u.name AS updated_by, mm.name AS member_name
						FROM `+userDbReplaceStr+`.member_loan AS m 
						LEFT JOIN `+userDB+`.user AS u ON m.req_user=u.id 
						LEFT JOIN `+userDbReplaceStr+`.member AS mm ON m.member_id=mm.id 
...
func insertMemberLoan(w http.ResponseWriter, r *http.Request) {
	execute(w, r, `INSERT INTO `+userDbReplaceStr+`.member_loan
			(member_id, member_loan_plan_id, req_date, rate, amount, charges, duration_months, grace_period_days, late_payment_charge, reject_cheque_penalty, note, status, req_user, rental) 
			VALUES(:member_id, :member_loan_plan_id, :req_date, :rate, :amount, :charges, :duration_months, :grace_period_days, :late_payment_charge, :reject_cheque_penalty, :note, :status, :req_user, :rental)`)
}
...
SET  member_loan_plan_id=:member_loan_plan_id, req_date=:req_date, rate=:rate, amount=:amount, charges=:charges, duration_months=:duration_months, grace_period_days=:grace_period_days, late_payment_charge=:late_payment_charge, reject_cheque_penalty=:reject_cheque_penalty, note=:note, rental=:rental, req_user=:req_user
						where id=(:id)`)
}
